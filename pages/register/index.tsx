...
import { FormState, RegisterFormProps } from "../../types/auth";
import { useDispatch } from "react-redux";
import { fetchRegister, setLoadingState } from "../../store/actions-creators/auth";
...
const RegisterFormSchema = yup.object({
  ...
  password: yup.string().required('You need to enter a password.').min(8),
  password2: yup.string().min(8).required().oneOf([yup.ref('password')], `Passwords don't match`),
}).required();
...
const Register = () => {
  ...
  const onSubmit = (data: RegisterFormProps) => {
    dispatch(fetchRegister({ email: data.email, fullname: data.fullname, password: data.password }))
  };


  const handleClose = () => {
    dispatch(setLoadingState(FormState.NEVER))
  }
  const { formState, registerError, user } = useTypedSelector(state => state.auth)
  const alert = FormState.NEVER === formState ? false : true
  const severity = FormState.SUCCESS === formState ? 'success' : 'error'

  React.useEffect(() => {
    if(FormState.SUCCESS === formState){
      setTimeout(() => {
        router.push('/')
      }, 1000)
    }
  ...
  return (
    <Box className={style.wrapper}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <InputLabel>What's your fullname?</InputLabel>
        <TextField
          margin="dense"
          id="fullname"
          label="fullname"
        ...
        <TextField
          ...
          {...register("password2")}
        />
        <Typography color="error">{errors.password2?.message}</Typography>
        <LoadingButton
        ...
        <Box display="flex" alignItems="center" justifyContent="center">
          <Typography color="GrayText" sx={{marginRight: 2}}>Have an account?</Typography>
